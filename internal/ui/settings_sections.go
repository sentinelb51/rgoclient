...
func (p *SettingsPage) cacheSection() []settingsGroup {
	...
	return []settingsGroup{
		...
		p.group("Usage", "",
			...
			p.numberRow("Video disk limit", "Videos fetched for playback; the ones watched longest ago are deleted first.",
				settings.VideoDiskMiB, minCacheMiB, maxDiskMiB, "MiB",
				func(s *config.Settings, v int) { s.Cache.VideoDiskMiB = v }),
