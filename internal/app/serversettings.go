...
// serverSettingsChannels lists the server's channels in sidebar order — the same
// walk the sidebar makes, so the page agrees with the column it covers. A channel
// the account cannot see is left out of both.
func (a *App) serverSettingsChannels() []ui.ServerChannelEntry {
	server, ok := a.store.Server(a.serverSettingsID)
	if !ok {
		return nil
	}

	entries := make([]ui.ServerChannelEntry, 0, len(server.Channels))
	add := func(channelID string) {
		...
		entries = append(entries, ui.ServerChannelEntry{
			ID:          channel.ID,
			Name:        channel.Name,
	...
	// Categorised first, in the order the server names them, then whatever no
	// category claimed — which is exactly how refreshChannelList walks them.
