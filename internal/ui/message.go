...
// ChannelNote is the strip under the message header saying what is going on in
// the channel below. A standing caption rather than a notice — what it says holds
// as long as the channel is open, so it cannot fade — and not the column's status
// line, which stands *in place of* messages a voice channel still has. Shortened
// rather than wrapped: the strip is fixed height and must not grow the column,
// and NewFillRow hands the ellipsis box the leftover width.
//
// A widget rather than a built subtree because a voice channel's caption changes
// with the call, and because the action slot at its end is what a voice channel
// is joined from — the one surface drawn only for one, built once and relaid out
// on every switch.
type ChannelNote struct {
	...
	// box is the ellipsis container around label, and full is what it was last
	// asked to say. ellipsisLayout rewrites label.Text during layout, so reading it
	// back would compare against a shortened caption rather than the real one.
	box  *fyne.Container
	full string
...
var _ fyne.Widget = (*ChannelNote)(nil)

// NewChannelNote builds the strip with a mark and its opening caption.
func NewChannelNote(mark fyne.Resource, text string) *ChannelNote {
	w := &ChannelNote{
		label:  newText(text, theme.Colors.ChannelNoteText, theme.Sizes.ChannelNoteTextSize),
		full:   text,
		action: container.NewStack(),
	}
	w.box = NewEllipsisText(w.label)

	side := theme.Sizes.ChannelNoteMarkSize
	icon := newScaledIcon(tintedIcon(mark, theme.Colors.ChannelNoteText), side)
	box := container.NewCenter(container.NewGridWrap(fyne.NewSize(side, side), icon))

	// The action is at the fixed end and the caption stretches, so a long sentence
	// shortens rather than pushing the button out of the strip.
	row := NewFillRow(2, box, HorizontalSpacer(theme.Sizes.ChannelNoteGap), w.box,
		HorizontalSpacer(theme.Sizes.ChannelNoteGap), container.NewCenter(w.action))

	padV, padH := theme.Sizes.ChannelNotePaddingV, theme.Sizes.ChannelNotePaddingH
	w.content = NewInset(row, padV, padV, padH, padH)
...
func (w *ChannelNote) CreateRenderer() fyne.WidgetRenderer {
	return widget.NewSimpleRenderer(w.content)
}

// Set changes the caption. A no-op on unchanged text: this is called on every
// channel switch and most of them say the same thing.
func (w *ChannelNote) Set(text string) {
	if w.full == text {
		return
	}
	w.full = text

	SetEllipsisText(w.box, text)
}

// SetAction puts a control at the strip's trailing end, or takes it away with
// nil. The slot is empty rather than hidden when there is nothing to offer —
// hiding a child reclaims nothing — so the caller relayouts the column after.
func (w *ChannelNote) SetAction(action fyne.CanvasObject) {
	w.action.Objects = nil
	if action != nil {
		w.action.Objects = []fyne.CanvasObject{action}
	}
